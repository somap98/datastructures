...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for(int i = 0; i < length-1; i++) {
            SNode left = new SNode(new Student(StdIn.readString(), StdIn.readString(), StdIn.readInt()), null);
            left.setNext(studentsInLine);
            studentsInLine = left;
        }
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        SNode left = musicalChairs;
        SNode delete = new SNode();

        for(int i = 0; i < size; i++) {
            if(i == random) {
                delete = ptr;
                
                left.setNext(ptr.getNext());
                if(size == 2) {
                    musicalChairs = left;
                    musicalChairs.setNext(musicalChairs);
                    ptr = musicalChairs;
        ...
        insertByHeight(delete.getStudent());
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        while(ptr!=null && ptr.getStudent().getHeight()<=studentToInsert.getHeight()) {
            left = ptr;
            ptr = ptr.getNext();
        }

        
        if(ptr == null && studentsInLine == null) {
            studentsInLine = new SNode(studentToInsert,null);
        } else if(ptr == null) {
            left.setNext(new SNode(studentToInsert, null));
            
        } else if(left == null) {
            left = new SNode(studentToInsert, studentsInLine);
            studentsInLine = left;
        } else{
            left.setNext(new SNode(studentToInsert, ptr));
        }
    ...
    public void seatMusicalChairsWinner () {
       ...
       o: for(int i = 0; i<seatingLocation.length; i++) {
            for(int j = 0; j<seatingLocation[0].length; j++) {
